...
are defined using four coefficients $\mathtt{a1}\le\mathtt{a2}\le\mathtt{a3}\le\mathtt{a4}$,
...
TO BE DONE... Problem statement... Given a fuzzy number $A$ 
and fixed \argument{knot.alpha} we seek for a piecewise linear
...
\paragraph{Finding best \argument{knot.alpha} for $\text{\argument{knot.n}}=1$ numerically.}
Let us depict the ``best'' $L_2$ distance as a function of $\alpha$,
...
plot(a, d, type='l', xlab='$\\alpha$', ylab='$D_{A}(\\alpha)$')
@
\end{center}

For $\text{\argument{knot.n}}=1$ we may find best \argument{knot.alpha} using numerical optimization.
It may be shown, see \cite{CoroianuETAL2012:plfnapprox},
