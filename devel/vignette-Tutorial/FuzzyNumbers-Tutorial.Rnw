...
if (as.integer(substring(FNver, nchar(FNver))) %% 2 == 0) {
...
\]
It may be shown that the solution to this problem
always exists and is unique, see \cite{CoroianuETAL2013:plfnapprox}
and \cite{CoroianuETAL2013:plfnapprox2}.

The \str{"{}NearestEuclidean"{}} \argument{method}
uses the algorithm described in \cite{CoroianuETAL2013:plfnapprox}
and \cite{CoroianuETAL2013:plfnapprox2}.
...
n <- 1:25
d <- matrix(NA, ncol=3, nrow=length(n))
...
for (i in seq_along(n))
{
   ...
   upbound <- alphacut(A, c(0,P1@knot.alpha,1))
   upbound <- abs(c(diff(upbound[,1]), diff(upbound[,2])))*sqrt(2)
   d[i,3] <- sqrt(2)*max(upbound)
}
...
par(mar=c(4,4,2,1))
matplot(n, d, type='l', xlab='$n$', ylab='$d(A, \\cdot)$', log="y", lty=c(1,2,4), col=c(1,2,4))
legend(x=15, y=5, c('$d(A, N^n(A))$', '$d(A, \\Pi^n(A))$', 'upper bound'),
...
It may be shown, see \cite{CoroianuETAL2013:plfnapprox},
