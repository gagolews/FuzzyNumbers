...
if (substring(FNver, nchar(FNver)) == "0") {
...
\begin{center}
{\LARGE\sf A Guide to the \package{FuzzyNumbers} {\Sexpr{FNver}} Package for \R}
...
They are available as the \func{approxInvert()}
...
plot(P1, type='b', xlab='$x$', ylab='$\\alpha$', from=0, to=5, xlim=c(0.5,4.5))
plot(P2, type='b', col=2, lty=2, pch=2, add=TRUE, from=0, to=5)
@
\end{center}
...
plot(P3, type='b', xlab='$x$', ylab='$\\alpha$', from=-1, to=9, xlim=c(0,8))
abline(h=alpha, col='gray', lty=2)
abline(v=P3['knot.left'], col='gray', lty=3)
abline(v=P3['knot.right'], col='gray', lty=3)
text(7.5, alpha, sprintf('$\\alpha=%g$', alpha), pos=3)
@
\end{center}
...
To depict FNs we use the \func{plot()} method,
which uses similar parameters as the \R-built-in
\func{curve()} function. If you are new to \R, you may wish
...
\noindent
Note that in result we always get a matrix with two columns.
...
\noindent Note that the vector has named elements (these ``labels''
...
plot(A, xlim=c(0,6), xlab='$x$', ylab='$\\alpha$')
...
plot(fapply(A, function(x) log(x+1)^0.5), xlab='$x$', ylab='$\\alpha$')
legend('topleft', '$\\sqrt{\\log(A+1)}$', lty=1)
@
...
Types available (argument \argument{type}): \str{"{}Euclidean"{}} (default),
...
The \str{"{}Naive"{}} method generates a trapezoidal FN with the same
...
The \str{"{}SupportCoreRestricted"{}} method was proposed in \cite{GrzegorzewskiPasternak2011:trapapproxsupcore}.
...
Currently only fixed \argument{knot.alpha} allowed.... TO BE DONE....
...
The \str{"{}Naive"{}} method generates a PLFN with the same
...
For \texttt{\argument{knot.n}==1} the method proposed in \cite{CoroianuETAL2012:plfnapprox}
is used. [WORK ON \texttt{\argument{knot.n}>1} ALMOST FINISHED]
...
\paragraph{Finding best \argument{knot.alpha} numerically.}
...
We may find best \argument{knot.alpha} using numerical optimization.
