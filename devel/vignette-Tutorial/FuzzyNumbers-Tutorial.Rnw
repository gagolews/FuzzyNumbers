...
package available on \textit{CRAN} we type\footnote{If the
title of this vignette is ``Guide to the FuzzyNumbers X.X-\textbf{devel} package'',
then some of the features presented here may be missing in the CRAN
release. Please, upgrade to the latest development version from
