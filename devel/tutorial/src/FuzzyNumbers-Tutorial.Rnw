...
legend(x=20, y=3.2, c('$d(A, N^n(A))$', '$d(A, \\Pi^n(A))$', 'upper bound'),
...
\cite{CoroianuETALXXXX:piecewise1suppcore}.
...
legend("topleft", legend=c("$A$", "$P_1$", "$P_2$"), lty=c(1,2,4), col=c(1,2,4), lwd=c(1,1,2))
@
\end{center}
...
\cite{CoroianuETALXXXX:piecewise1suppcore} for discussion.
...
Da <- sapply(a, D)
plot(a[-c(1,length(a))], Da[-c(1,length(a))], xlab="$\\alpha$", ylab="$D(\\alpha)$",
